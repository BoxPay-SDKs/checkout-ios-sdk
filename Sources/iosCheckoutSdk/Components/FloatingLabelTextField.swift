...
struct FloatingLabelTextField: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if showSuggestions, let suggestions = suggestions {
                List(filteredOptions, id: \.self) { item in
                    Text(item)
                        .onTapGesture {
                            text = item
                ...
                .frame(height: CGFloat(min(150, filteredOptions.count * 44)))
                .listStyle(.plain)
            }
        }
        .onChange(of: text) { newValue in
            guard let suggestions = suggestions else { return }
            filteredOptions = suggestions.filter { $0.lowercased().contains(newValue.lowercased()) }
            showSuggestions = !filteredOptions.isEmpty
        }
    }
}
